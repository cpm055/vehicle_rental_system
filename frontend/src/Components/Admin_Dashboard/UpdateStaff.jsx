  ...
  console.log("User ID:", userId);
    ...
    useEffect(() => {
        ...
        async function fetchUserData() {
          try {
            const response = await axios.get(`http://localhost:5000/api/staff/${userId}`);
      ...
      const handleSubmit = async (e) => {
        e.preventDefault();
        try {
          const response = await axios.put(`http://localhost:5000/api/staff/${userId}`, formData);
          if (response.data.success) {
            toast.success('User updated successfully!');
            setTimeout(() => {
                navigate("/a-dashboard/manStaff");
            }, 1000);
          } else {
