...
function ManageStaff() {
    ...
    async function fetchUsers() {
      try {
        const response = await axios.get('http://localhost:5000/api/staff'); // Get users from the backend
    ...
    const handleDelete = async (id) => {
      ...
      function handleAddStaff() {
        navigate('/a-dashboard/addStaff')
      }
    
      const handleUpdateUser = (userId) => {
        navigate(`/a-dashboard/updateStaff/${userId}`);
      };

      // Generate PDF Report
          const generatePDF = () => {
            const doc = new jsPDF();
            doc.text("Staff Management Report", 20, 10);
